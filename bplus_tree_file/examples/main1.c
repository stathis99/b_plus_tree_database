...
void printFiles(int fd){
			  ...
			  BF_Block_Init(&block);
				...
				for(int i=1;i<blocks_num;i++){
					...
					data = BF_Block_GetData(block);
					printf(" %c ",data[0]);
					if(*data == 'I'){ // an einai eurethrio
						int counter = *(data+1);

						printf(" %d ",*(data+1));
						printf("%d ",*(data+1+sizeof(int)));

						for(int j=0;j<counter;j++){
							if(type[0] == 'i'){
								printf(" %d ",*(data+1+2*sizeof(int)+j*(sizeof(int)+length[0])));
								printf(" %d ",*(data+1+2*sizeof(int)+length[0]+j*(sizeof(int)+length[0])));
							}
							else if(type[0] == 'c'){
								printf(" %s ",(data+1+2*sizeof(int)+j*(sizeof(int)+length[0])));
								printf(" %d ",*(data+1+2*sizeof(int)+length[0]+j*(sizeof(int)+length[0])));
							}
							else if(type[0] == 'f'){
								float* myfloat = data+1+2*sizeof(int)+j*(sizeof(int)+length[0]);
								printf(" %.2f ",*myfloat);
								printf(" %d ",*(data+1+2*sizeof(int)+length[0]+j*(sizeof(int)+length[0])));
							}
						}

					}else{ //apo edw kai katw einai h periptwsh tou leaf
						int counter = *(data+1);
						printf(" %d ",counter);
						printf("%d ",*(data+1+sizeof(int))); //o dikths
						for(int j=0;j<counter;j++){
							if(type[0] == 'i'){

								if(type[1] == 'c'){
									printf(" %d ",*(data+1+2*sizeof(int)+j*(length[1]+length[0])));
									printf(" %s ",(data+1+2*sizeof(int)+length[0]+j*(length[1]+length[0])));
								}

							}
							else if(type[0] == 'c'){

								printf(" %s ",(data+1+2*sizeof(int)+j*(length[1]+length[0])));
								if(type[1] == 'f'){
									float* myfloat = data+1+2*sizeof(int)+length[0]+j*(sizeof(int)+length[0]);
									printf(" %.2f ",*myfloat);
								}else{
									printf(" %d ",*(data+1+2*sizeof(int)+length[0]+j*(sizeof(int)+length[0])));
								}
								...
								if(type[1] == 'c'){
									float* myfloat = data+1+2*sizeof(int)+j*(length[1]+length[0]);
									printf(" %.2f ",*myfloat);
									printf(" %s ",(data+1+2*sizeof(int)+length[0]+j*(length[1]+length[0])));
								}
							}
...
//	 ********************************************************************************/
	if ((ivalue = (int*) AM_FindNextEntry(scan1)) != NULL)
		...
		sprintf(errStr,"ANEMONH was not found... (Note: This is the correct behaviour!)");
		AM_PrintError(errStr);
	}
																								printf("ANEMONH was not found... (Note: This is the correct behaviour!)\n");
